...
아래는 구현 후 **실측치**다. 실제 `store.MemoryStore`와 실제 이벤트 페이로드 타입에 측정된 트래픽 프로파일을 흘려 `json.Marshal` 바이트를 잰 값이며, 설계 시 추정치가 아니다(추정치와의 차이는 §9 참고).

**3배 목표 규모 — 600토픽 / 1,400 msg/s**

| | 변경 전 | 변경 후 |
|---|---|---|
| 트리 | 992.9 KB × 20/s = **19.39 MB/s** | 165.0 KB × 2/s = **0.32 MB/s** (변경 시에만) |
| 메시지 (로봇 1대 선택) | 108.2 KB × 20/s = **2.11 MB/s** | 1.6 KB × 20/s = **0.03 MB/s** |
| 메시지 (선택 없음) | 위와 동일 | **0 B** |
| `History()` 폴링 | 20회/s × 200메시지 | 선택 시 1회 |
| **합계** | **21.51 MB/s** | **0.35 MB/s** — 61배 감소 |

**측정 규모 — 104토픽 / 467 msg/s** (실제 브로커 프로파일)

| | 변경 전 | 변경 후 |
|---|---|---|
| 트리 | 172.4 KB × 20/s = 3.37 MB/s | 28.9 KB × 2/s = 0.06 MB/s |
| 메시지 (로봇 1대 선택) | 35.5 KB × 20/s = 0.69 MB/s | 1.6 KB × 20/s = 0.03 MB/s |
| **합계** | **4.06 MB/s** | **0.09 MB/s** — 46배 감소 |

emit 상한도 실제 코드로 확인했다: 한 flush에 focus 매칭 250건이 들어오면 100건만 남고 150건이 dropped로 보고된다(= 2,000 msg/s 천장).
...
| 기준 | 상태 |
|---|---|
| 브릿지 트래픽 before/after 측정 | ✅ 완료 — §3.4에 기록 |
| 유휴(선택 없음) 상태에서 `mqtt:messages` emit 0건 | ✅ 코드로 보장 — `flush`가 `focus == ""`이면 `EventsEmit` 이전에 return |
| emit 상한 동작 | ✅ `CapBatch` 단위 테스트 + §3.4 실측 |
| 토픽 선택 클릭 → 화면 반영 200ms 이내 | ⏳ 실브로커 + GUI 필요 — 수동 체크리스트 |
| 1,400 msg/s 부하에서 트리 스크롤·클릭 끊김 없음 | ⏳ 실브로커 + GUI 필요 — 수동 체크리스트 |

## 9. 설계 추정치와 실측치의 차이

설계 단계의 브릿지 비용 추정은 두 방향으로 빗나갔다. 기록해 둔다.

- **트리 스냅샷 "변경 후"를 과소평가했다.** 600토픽 기준 ~60 KB로 잡았으나 실측 165 KB다. leaf의 payload만 계산했고, 모든 노드에 붙는 `fullTopic` 문자열과 RFC3339 `lastSeen` 타임스탬프를 빠뜨렸다.
- **"변경 전"은 과소평가했다.** 15.8 MB/s로 잡았으나 실측 19.39 MB/s다.

결론은 유지된다 — 합계 21.51 → 0.35 MB/s, 61배 감소. 다만 "~130 KB/s"라고 적었던 목표치는 실제 350 KB/s이므로, 이 문서의 수치를 인용할 때는 §3.4의 실측 표를 쓸 것.
