...
- [ ] **연결 직후 msg/s 워밍업 관찰** — `Rates()`는 창을 항상 5초로 보고 나누므로, 연결 후 첫 5초는 값이 실제보다 낮게 시작해 올라온다. 이게 실사용에서 거슬리는지 판단할 것. 거슬린다면 실제 경과한 버킷 수로 나누도록 바꾼다(`internal/app/rate.go`) — 단 스펙 테스트 `TestRateCounterAveragesOverWindow`의 계약이 함께 바뀐다. Task 4 코드 리뷰에서 제기됐고, 근거 없이 계약을 바꾸지 않으려 여기로 미룬 항목이다
